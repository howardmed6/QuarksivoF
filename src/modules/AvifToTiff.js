...
const validateImageBuffer = async (imageBuffer) => {
    ...
    try {
        ...
        // Verificar que sea realmente AVIF o que Sharp pueda procesarlo
...
/**
 * Convierte AVIF a TIFF con opciones de compresión
 * @param {Buffer} avifBuffer - Buffer de imagen AVIF
 * @param {Object} options - Opciones de conversión TIFF
 * @returns {Promise<Buffer>} - Buffer de TIFF
 */
const convertAvifToTiff = async (avifBuffer, options = {}) => {
    const {
        compression = 'lzw',
        quality = 80,
    ...
    try {
        let sharpInstance = sharp(avifBuffer);
        ...
        throw new Error(`Error convirtiendo AVIF a TIFF: ${error.message}`);
    }
};

/**
 * Procesa conversión completa de AVIF a TIFF con opciones
 * @param {Buffer} avifBuffer - Buffer de imagen AVIF
 ...
 */
const processAvifToTiff = async (avifBuffer, processingOptions = [], conversionParams = {}) => {
    try {
        // Validar que Sharp puede procesar el buffer
        const isValidImage = await validateImageBuffer(avifBuffer);
        if (!isValidImage) {
            throw new Error(`El archivo no es una imagen AVIF válida o está corrupto`);
        }

        const originalMetadata = await sharp(avifBuffer).metadata();
        const originalSize = avifBuffer.length;

        console.log(`📥 Procesando AVIF a TIFF: ${originalMetadata.width}x${originalMetadata.height}, ${(originalSize/1024/1024).toFixed(2)}MB`);
        console.log(`📋 Formato detectado por Sharp: ${originalMetadata.format}`);

        let processedBuffer = avifBuffer;
        ...
        console.log(`🔄 Convirtiendo AVIF a TIFF con compresión ${tiffOptions.compression}...`);
        const tiffBuffer = await convertAvifToTiff(processedBuffer, tiffOptions);
        ...
        console.error(`❌ Error en proceso AVIF->TIFF:`, error.message);
        throw new Error(`Error en proceso AVIF->TIFF: ${error.message}`);
    }
};
