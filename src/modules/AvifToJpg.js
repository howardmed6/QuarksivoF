 ...
 */
const convertAvifToJpg = async (imageBuffer, options = {}) => {
    const {
        quality = 90,
        progressive = false,
        mozjpeg = true,
    ...
    try {
        ...
        // AVIF puede tener transparencia, manejarla como PNG
        ...
        throw new Error(`Error convirtiendo AVIF a JPG: ${error.message}`);
    }
};
...
const validateAvifImage = (imageBuffer) => {
    ...
    // Verificar magic bytes de AVIF (ftypavif en posición 4-12)
    const ftypSignature = imageBuffer.slice(4, 8).toString('ascii');
    const avifSignature = imageBuffer.slice(8, 12).toString('ascii');
    
    return ftypSignature === 'ftyp' && avifSignature === 'avif';
};

/**
 * Procesa conversión completa de AVIF a JPG
 * @param {Buffer} imageBuffer - Buffer de imagen AVIF
 ...
 */
const processAvifToJpg = async (imageBuffer, processingOptions = [], conversionParams = {}) => {
    try {
        ...
        console.log(`📥 Procesando AVIF: ${originalMetadata.width}x${originalMetadata.height}, ${(originalSize/1024/1024).toFixed(2)}MB`);
        ...
        const jpgBuffer = await convertAvifToJpg(processedBuffer, conversionParams.jpgOptions);
        ...
        throw new Error(`Error en proceso AVIF->JPG: ${error.message}`);
    }
};
